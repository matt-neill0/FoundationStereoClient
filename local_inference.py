...
class LocalEngineRunner:
    ...
    def __init__(
        ...
        # Track which source is currently used for preview frames so GUI tooling can
        # surface this information without probing internal state dynamically.
        # Possible values: "left" (default left camera), "realsense_color"
        # (fresh RGB frame from the RealSense colour sensor) and "realsense_cache"
        # (stale-but-recent colour frame when no fresh data is available).
        self._preview_frame_source: str = "left"
    ...
    def _result_metadata(self) -> Dict[str, Any]:
        meta: Dict[str, Any] = {
            ...
            "preview_frame_source": getattr(self, "_preview_frame_source", "left"),
        }
    ...
    def _latest_color_preview(self) -> Optional[np.ndarray]:
        ...
        self._preview_frame_source = "realsense_color"
        return frame
    ...
    def _preview_display_frame(self, left_bgr: np.ndarray) -> np.ndarray:
        ...
        if self._last_color_preview is not None:
            self._preview_frame_source = "realsense_cache"
            return self._resize_frame(self._last_color_preview)
        self._preview_frame_source = "left"
        return left_bgr
